...
// const DetailsScreen: React.FC<Props> = ({product = mock.product}) => {
  const DetailsScreen: React.FC<Props> = ({product}) => {
    const [selectedIMG, setSelectedIMG] = useState(product.pictures[0].url)
    ...
    // console.log(reviewsAmount, reviewsRating);
  return (
    <Box bg="rgba(0,0,0,.1)" p={4}>
      <Container maxWidth="container.xl" p={0} fontSize="sm">
        ...
        <Stack mb={4} direction="row" justify="space-between">
      ...
      <Container p={4} bg="white" maxWidth="container.xl" borderRadius="5px" boxShadow="rgba(0, 0, 0, 0.24) 0px 3px 8px">
        <Stack direction="row" spacing={12} >
          ...
          <Stack pl={4} direction="column" flex={1}>

            {/* carrusel */}
            <Stack direction="row" align="flex-start">
              <Stack direction="column" spacing={4}>
                {product.pictures.map( (p, i) => { 
                  return <Image key={i} onClick={()=> setSelectedIMG(p.url)} src={p.url} border="1px gray solid" p={1} w={50} />
                })}
              </Stack>
              <Flex flex={1} justify="center" align="center">
                <Image w={450} objectFit="cover" src={selectedIMG} />
              </Flex>
            </Stack>
            ...
            <Stack spacing={3}>
              ...
              {/*DESCRIPCION*/}
              <Divider h="1px" bg="grisecito" w="100%" orientation="horizontal"  />
              <Text as="h1" fontSize="2xl" fontWeight="bold">Descripción</Text>
              <Text mb={12} fontSize={20} color="#666"> ******** HOLA! SOMOS DR.CLICK ********
